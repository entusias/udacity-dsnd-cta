...
class Line:
    ...
    def process_message(self, message):
        ...
        if "org.chicago.cta.stations.table.v1" == message.topic():  # stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal(f"bad station? {value}, {e}")
        elif "org.chicago.cta.station.arrivals." in message.topic():  # arrival topic
            self._handle_arrival(message)
        elif "TURNSTILE_SUMMARY" == message.topic():  # KSQL Turnstile Summary Topic
