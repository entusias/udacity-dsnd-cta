...
def run_server():
    ...
    consumers = [
        KafkaConsumer(
            "org.chicago.cta.weather.v1",  # weather topic
            weather_model.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
            "org.chicago.cta.stations.table.v1",  # faust topic
            lines.process_message,
            offset_earliest=True,
            is_avro=False,
        ),
        KafkaConsumer(
            "^org.chicago.cta.station.arrivals.",  # station arrival topics
            lines.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
            "TURNSTILE_SUMMARY",  # KSQL table topic
            lines.process_message,
            offset_earliest=True,
            is_avro=False,
        ),
